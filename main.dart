...
class Jogo {
  ...
  // PONTOS NA MESA (ao vivo): bônus das canastras + valor das cartas já baixadas.
  // Serve pra mostrar a soma correndo durante a rodada (sem descontos/bônus de fim).
  int pontosMesa(String dupla) {
    int total = 0;
    for (final meld in jogosDupla[dupla]!) {
      if (meld.length >= 7) {
        final res = validarSequencia(meld);
        if (res['valido'] == true) {
          switch (res['tipo']) {
            case 'as_a_as': total += 1000; break;
            case 'de_500': total += 500; break;
            case 'limpa': total += 200; break;
            case 'suja': total += 100; break;
          }
        }
      }
      for (final c in meld) total += _pontos(c);
    }
    return total;
  }
...
class _MesaScreenState extends State<MesaScreen> {
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
        ...
        Padding(
          ...
          child: Row(children: [
            ...
            const Spacer(),
            // PLACAR ao vivo: placar oficial + pontos já na mesa nesta rodada (soma correndo)
            Text('NÓS ${_j.placar['nos']! + (_j.rodadaEncerrada ? 0 : _j.pontosMesa('nos'))}', style: const TextStyle(color: Color(0xFF8FE0B0), fontSize: 16, fontWeight: FontWeight.w900)),
            const Text('  ×  ', style: TextStyle(color: Color(0xFF8A7A54), fontSize: 13, fontWeight: FontWeight.w700)),
            Text('ELES ${_j.placar['eles']! + (_j.rodadaEncerrada ? 0 : _j.pontosMesa('eles'))}', style: const TextStyle(color: Color(0xFFE7B7A6), fontSize: 16, fontWeight: FontWeight.w900)),
          ]),
        ),
      ]),
    );
      ...
      child: Stack(children: [
        ...
        Positioned.fill(
          child: Column(children: [
            Expanded(flex: 40, child: _areaJogo('eles')),
            Expanded(flex: 20, child: _faixaCentral()),
            Expanded(flex: 40, child: _areaJogo('nos')),
          ]),
        ),
        // dica curta de jogo (não é bloco explicativo — some quando não há o que dizer)
        Positioned(left: 8, right: 8, bottom: 106, child: IgnorePointer(child: Text(_dica, textAlign: TextAlign.center,
        ...
        Positioned(left: 0, right: 0, bottom: 0, height: 98, child: _hand()),
  ...
  Widget _areaJogo(String dupla) {
    ...
    final aL = eles ? 1 : 2;
    final aR = eles ? 3 : 0;
    // dupla NÓS: avatares sobem um pouco (encostam na faixa central), sem cobrir as cartas maiores
    final avTop = eles ? 2.0 : -14.0;
    final padTop = eles ? 52.0 : 36.0;
  ...
  Widget _meldCards(List<Carta> cartas) {
    // cartas MAIORES e mais SOBREPOSTAS (leque apertado) — mostram o canto valor+naipe
    const peek = 22.0, cw = 44.0, ch = 64.0;
  ...
  Widget _mcard(Carta c) => Container(
        ...
        decoration: const BoxDecoration(
          ...
          boxShadow: [BoxShadow(color: Color(0x77000000), blurRadius: 2, offset: Offset(1, 0))],
        ),
        clipBehavior: Clip.antiAlias,
  ...
  Widget _back({bool destaque = false}) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: destaque ? _mGoldHi : const Color(0x66EFB94A), width: destaque ? 2.4 : 1.2),
        boxShadow: destaque ? const [BoxShadow(color: Color(0x99EFB94A), blurRadius: 9)] : const [BoxShadow(color: Color(0x66000000), blurRadius: 3, offset: Offset(0, 1))],
      ),
      clipBehavior: Clip.antiAlias,
  ...
  Widget _lixoCarta(Carta c, bool destaque) {
    return Container(
      width: 58, height: 82,
      decoration: BoxDecoration(borderRadius: BorderRadius.circular(8), border: Border.all(color: destaque ? _mGoldHi : const Color(0x55000000), width: destaque ? 2.4 : 1), boxShadow: destaque ? const [BoxShadow(color: Color(0x99EFB94A), blurRadius: 9)] : const [BoxShadow(color: Color(0x55000000), blurRadius: 3, offset: Offset(0, 1))]),
      clipBehavior: Clip.antiAlias,
      child: Image.asset(_cartaAsset(c), fit: BoxFit.fill),
    );
  }
  ...
  Widget _ghost({bool destaque = false}) => Container(
        width: 58, height: 82,
        decoration: BoxDecoration(color: const Color(0x0DFFFFFF), borderRadius: BorderRadius.circular(8), border: Border.all(color: destaque ? _mGoldHi : const Color(0x3AFFFFFF), width: destaque ? 2.4 : 1.4)),
      );
  ...
  Widget _hand() {
    ...
    // Cartas em LEQUE (sobrepostas, como uma mão de verdade). A mão fica presa à base:
    //  - FORA da vez: desce 50% da altura (metade some, cortada pela borda);
    //  - NA vez: sobe inteira. Selecionar levanta a carta e traz pra frente.
    const cw = 54.0, ch = 80.0, peek = 30.0, lift = 18.0;
    const boxH = ch + lift; // folga no topo pra carta selecionada subir
    final ativo = _minhaVezAtiva;
    final larguraLeque = (n - 1) * peek + cw;
    Widget cardAt(int i, bool sel) => Positioned(
          left: i * peek,
          top: sel ? 0 : lift, // selecionada sobe (fica 'lift' acima das outras)
          child: GestureDetector(
            onTap: ativo ? () => _tapCard(i) : null,
            child: SizedBox(width: cw, height: ch, child: _handCard(mao[i], sel)),
          ),
        );
    // não-selecionadas por baixo; selecionadas por cima (z maior) e levantadas
    final leque = SizedBox(
      width: larguraLeque, height: boxH,
      child: Stack(clipBehavior: Clip.none, children: [
        for (int i = 0; i < n; i++) if (!_sel.contains(i)) cardAt(i, false),
        for (int i = 0; i < n; i++) if (_sel.contains(i)) cardAt(i, true),
      ]),
    );
    final conteudo = LayoutBuilder(builder: (context, cons) {
      return SingleChildScrollView(
        ...
        child: larguraLeque + 24 < cons.maxWidth
            ? SizedBox(width: cons.maxWidth - 24, child: Center(child: leque))
            : leque,
      );
    });
    return SizedBox(
