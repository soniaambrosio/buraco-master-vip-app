...
void main() {
  ...
  group('carga — abrir a tela não é perguntar de novo', () {
    test('`garantir` consulta uma vez; as nove seguintes não consultam', () async {
  ...
  group('resposta vencida — quem chega por último não é quem manda', () {
    ...
    test('falhar no "carregar mais" NÃO derruba a lista que está na tela', () async {
  ...
  group('troca de sessão — nada de A sobra para B', () {
    ...
    test('a vista de UM jogador em voo não atravessa a troca', () async {
      ...
      expect(await velha, isNull, reason: 'a resposta da conta que saiu passou');
  ...
  group('busca — o termo, o vencido e o vazio', () {
    ...
    test('os resultados anteriores FICAM enquanto a nova consulta corre', () async {
  ...
  group('ação aceita — subtrai, relê, e NUNCA adiciona', () {
    ...
    test('aceitar NÃO adiciona ninguém à lista de amigos', () async {
      ...
      expect(
        leitor.amigos.itens.map((e) => e.publicId),
        ['P9'],
    ...
    test('a vista devolvida vem da AUTORIDADE, e não do desfecho', () async {
      ...
      t.respostaDaAcao = const DesfechoSocial(repeticao: false, estado: 'amigos');
    ...
    test('a releitura que falha vira "não sei" — nunca uma relação deduzida', () async {
      await leitor.garantir(QualLista.recebidas);
      ...
      expect(
        r.vista,
        isNull,
        reason: 'sem resposta da autoridade, a relação foi deduzida do desfecho',
      );
    ...
    test('`repeticao` chega como sucesso, e não como erro', () async {
      t.respostaDaAcao = const DesfechoSocial(repeticao: true, estado: 'amigos');
    ...
    test('a ação recusada PROPAGA a falha — não some em silêncio', () async {
      ...
      await expectLater(
        ...
        throwsA(
          isA<FalhaSocial>().having((e) => e.recusa, 'recusa', 'limiteDeAmigos'),
        ),
      );
    });
    ...
    test('a linha da BUSCA é trocada pela vista nova da autoridade', () async {
      ...
      expect(itens, hasLength(2), reason: 'a lista de resultados mudou de tamanho');
  ...
  group('vistaDe — a relação de UM, sem cache', () {
    test('dois pedidos simultâneos do mesmo id compartilham a chamada', () async {
      t.manual = true;
