...
void main() {
  ...
  group('INT-08 — encerramento por autoridade', () {
    ...
    test('a ordem viaja no desfecho para a auditoria', () async {
      ...
      // `impressaoPartida`, e não `impressao`: na consolidação do contrato de
      // encerramento ficou decidido que a identidade do desfecho inclui as
      // canastras limpas, porque elas decidem desempate de torneio. Este teste
      // nasceu antes dessa decisão e apontava para a impressão do `Jogo` — que
      // não distingue dois desfechos de mesmo placar e canastras diferentes.
      expect(d.impressaoEstado, motor.impressaoPartida);
    });
  });
