...
/// E-mail SINTÉTICO. `.invalido` é um TLD que não existe e nunca vai existir —
/// nenhuma pessoa real é alcançável por este endereço.
///
/// Aqui havia o e-mail pessoal de verdade da dona do projeto. Numa suíte que
/// prova redação de dado pessoal, isso é a própria falha que ela existe para
/// impedir: o endereço ficaria versionado, indexado e visível em qualquer log
/// de CI que ecoasse a linha do teste.
const String kEmailSintetico = 'jogadora.exemplo@exemplo.invalido';
...
void main() {
  ...
  group('1. endereço do servidor por perfil de build', () {
    ...
    test(
      'endereço inválido vira estado seguro, não crash nem spinner',
      () async {
  ...
  group('3. credencial: válida, recusada, vencida e renovação concorrente', () {
    test('token válido autentica e libera a fila', () async {
      ...
      await c.assentar();
      expect(
        c.canal.doTipo('criarMesa'),
        isEmpty,
        reason: 'ainda não autenticou',
      );
    ...
    test(
      'authExpirou renova UMA vez, mesmo repetido — sem tempestade',
      () async {
        final c = _Cenario(token: kToken);
        await c.conectar();
        await c.servidorAceita();
    ...
    test('nenhum comando carrega campo de identidade', () async {
      ...
      for (final msg in c.tudoQueSaiu) {
        for (final campo in proibidos) {
          expect(
            msg.containsKey(campo),
            isFalse,
            reason:
                'o campo "$campo" reapareceu em ${msg['tipo']} — '
  ...
  group('5. redação de segredos', () {
    ...
    test('e-mail e purchaseToken não sobrevivem', () {
      expect(
        redigir('user $kEmailSintetico'),
        isNot(contains(kEmailSintetico)),
      );
      expect(
        redigir('{"purchaseToken":"gpa.9999-8888"}'),
    ...
    test(
      'motivo vindo do servidor é redigido antes de chegar à tela',
      () async {
        final c = _Cenario(token: kToken);
        await c.conectar();
        await c.servidorAceita();
  ...
  group('6. queda, reconexão, limite e cancelamento', () {
    ...
    test(
      'logout derruba a conexão e apaga o que era da conta anterior',
      () async {
        final c = _Cenario(token: kToken);
        await c.conectar();
        await c.servidorAceita();
        ...
        expect(
          c.servico.visao,
          isNull,
          reason: 'projeção era da conta que saiu',
        );
    ...
    test('depois do logout, reconectar não reentra na mesa antiga', () async {
      ...
      expect(
        c.canais.last.doTipo('entrarMesa'),
        isEmpty,
  ...
  group('7. uma conexão por contexto — sem socket órfão', () {
    ...
    test(
      'comandos em rajada antes de conectar não multiplicam socket',
      () async {
        final c = _Cenario(token: kToken);
        c.servico.criarMesa(apelido: 'Sônia');
        c.servico.iniciarPartida();
