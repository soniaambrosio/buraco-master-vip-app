...
conferir_suite() {
  ...
  if [ -z "$exige_lista" ]; then
    ...
    exiges_do_gate=$((exiges_do_gate + $(printf '%s\n' "$exige_lista" | grep -c .)))
  fi
...
CONTRATO_EXIGE_ALVO
  ...
  #
  # Sobre a SOMA dos `exige` das suites do gate, pela mesma razao que
  # `PISOS_PROVAS`: desde a OS 40-C1 um gate pode declarar mais de uma `suite`, e
  # os literais pertencem a suite acima deles. Contar so a ULTIMA suite deixaria
  # o piso valendo por um pedaco do contrato — e um gate de uma suite so, que e o
  # caso de todos os nomeados nesta regua hoje, se comporta exatamente como antes.
  piso="$(piso_de "$PISOS_EXIGE" "$chave")"
  if [ -n "$piso" ]; then
    if [ "$exiges_do_gate" -lt "$piso" ]; then
      erro "o contrato de '$chave' declara $exiges_do_gate 'exige' e o piso e $piso — o contrato encolheu"
    else
      printf 'ok   piso       %-12s exige %s >= %s\n' "$chave" "$exiges_do_gate" "$piso"
    fi
  fi
